...
public class LargestSubArray01Pair1 {
		...
		public static void main(String[] args) {
			LargestSubArray01Pair1 sub = new LargestSubArray01Pair1(); 
...
3) There are two cases, the output subarray may start from 0th index or may start from some other
...
problem is equivalent to finding two indexes i & j in sumleft[] such that sumleft[i] = sumleft[j]
 and j-i is maximum. To solve this, we can create a hash table with size = max-min+1 where min
  is the minimum value in the sumleft[] and max is the maximum value in the sumleft[]. The idea 
  is to hash the leftmost occurrences of all different values in sumleft[]. The size of hash is
   chosen as max-min+1 because there can be these many different possible values in sumleft[]. 
...
6) To fill and use hash[], traverse sumleft[] from 0 to n-1. If a value is not present in hash[],
 ...
 index of sumleft[] and previously stored value in hash[]. If this difference is more than
  maxsize, then update the maxsize.
